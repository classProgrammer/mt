...
A conversation end is when the underlying task has been achieved.
...
It's important to keep in mind that a chatbot can mimic intelligence, human dialog, 
and personality but is often limited to specific functionality \cite{kane2016role, dutta2017developing}.
% chatbot categories
Three major chatbot types are present in the literature.
...
A chatbot system in the refund desk domain offers only the functionality necessary to get and collect refund information and to issue a refund. 
If the bot is asked something else no useful response can be expected.
...
The most limited approach is the menu-driven approach since the options are given and a user only inputs numbers to select a functionality.
...
The training data is used to train natural language capabilities and 
...
One simple rule could be that a person's name is required as input and consists of a first and last name.
% chatbot tasks
A chatbot has two major tasks.
It needs to classify the intention of the user called intent and extract the relevant pieces of information 
from the message.
A relevant piece of information is called an entity.
An entity has a type and can be e.g. a date or the name of a person.
...
The frameworks can be divided into cloud-based and local solutions.
...
The developer is dependent on the provider and the training and analysis are done in the cloud of the provider. This needs to be kept in mind when dealing with sensitive information like bank accounts.
...
In Table \ref{tab:example_sentences} the slots which need to be filled for a hotel reservation could be the hotel name, start and end date, and a name for the 
reservation.
...
Chatbots seem like intelligent systems but most often they are designed for specific tasks
...
A chatbot system is meant to reduce the workload of employees or serve a large number of users.
...
domain then the domain-specific approach works best (FAQ chatbot). In the FAQ example, the domain consists of all questions and answers of the FAQ page.
...
The open-ended approach is suitable when no specific data needs to be collected and a general conversation needs to be held with the users. 
...
"Tell my employee I am sick and won't come to work today" since this would require domain-specific data and actions.
...
A threshold defines the minimum score the system has to detect before an action is executed.
All values below the threshold are handled by the fallback action e.g. a generic response like 
...
The sentences listed as training phrases and the response lists are utterances. 
...
Real user input from e.g. e-mails, phone conversations, or online resources can be used to get a large amount of training data \cite{singhbuilding}.    
...
the second is the shirt as an item, and the third one the size.
...
In sentence five May is the month and needs to be treated as a date whereas in sentence six of Table \ref{tab:example_sentences} 
it's the last name and needs to be treated as a person entity. 
A named entity is a noun phrase relating to individuals.
Classic individuals/named entities are person (Bill Gates), location (New York), and organization (Google) \cite{geyer2016named}.
...
Alternatives are picked to keep the conversation interesting for the users especially
...
The "sad" path handles the behavior when something doesn't work out.
...
These "sad" paths need to be handled in separate stories.
...
The range of the f-score, precision, and recall values is zero to one where zero is the worst possible 
result and one is a perfect result.
The precision shows how many retrieved elements are relevant.
The recall shows hom many relevant items have been retrieved.
The f-score can be seen as the average of precision and recall.
In the medical area the recall is more important than the precision because the goal is 
to correctly identify as many sick people as possible.
It's not that important if people are false positive since further checks are always 
possible to verify the test result.
In the are of chatbots neither false negatives nor false positives are desireable hence the 
f-score is the most relevant value.
The precision is based on the selected elements (TP + FP) and the recall is based on the relevant elements (TP + \textnormal{text}).
Figure \ref{fig:f-score} from the appendix can be used as a visual aid for a better understanding.
