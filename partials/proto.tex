...
- name: FallbackPolicy
    nlu_threshold: 0.4
    core_threshold: 0.3
    fallback_action_name: utter_did_not_understand
\end{lstlisting} 
...
The story in \ref{lst:rasa_sickness_story} implements the description from above.
The next step is the confirmation that the entered data is correct.
For this a confirmation action is needed.
There are two possible values yes and no.
For this rasa offers the boolean slot type which is either true or false.
