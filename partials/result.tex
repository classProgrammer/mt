...
like \citet{singhbuilding} said, the best evaluation criteria is if a task can be achived with a technology or not.
The goal is to implement the dialogs shown in Figure \ref{fig:conversationflow}.
The result of this evaluation is a binary criteria which is either true when the implementation was possible or false
if it wasn't possible.
The prototype implmenetations in Chapter \ref{chap:proto} shown that the dialogs can be 
...
% required tools
As measurment criteria the required tools will be used.
To setup Watson Assistant, Dialogflow, and LUIS a web-browser is needed.
Those three technologies require nothing else since the development environment,
files, date are built into the websites of the technologies.
...
A local setup requires a pyton environment and a package manager like pip.
...
% setup time
As second measurment criteria for setup complexity a setup time ranking will be used.
...
Another measurement criteria for learnability is the amount of predefined entities.
If a predefined entity is available for a proble a developer doesn't need to 
